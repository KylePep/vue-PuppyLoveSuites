<template>
  <div v-if="isSmallScreen" class="d-flex flex-column justify-content-center align-items-center">
    <div v-for="day in longDays" :key="day"
      :class="[day.split('|')[0] == 'Sunday' ? 'bg-light' : 'bg-white', day.split('|')[0] == 'Sunday' ? 'border-bottom-1 rounded-top-1' : 'border-top-0', day.split('|')[0] == 'Saturday' ? 'rounded-bottom-1' : '']"
  ...
  <div v-else class="d-flex flex-column flex-grow-1 align-items-center">
    <div class="d-flex flex-row w-100">
      <div v-for="day in longDays" :key="day"
        :class="[day.split('|')[0] == 'Sunday' ? 'bg-light' : 'bg-white', day.split('|')[0] == 'Sunday' ? 'border-start-2 rounded-start' : 'border-start-0', day.split('|')[0] == 'Saturday' ? 'rounded-end' : '']"
